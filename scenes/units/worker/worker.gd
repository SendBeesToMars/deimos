...
@onready var farm: Area2D = get_tree().get_nodes_in_group("farm")[0]
...
func harvest(harvest_node_info: Dictionary[String, Dictionary], update_text: Callable):
	...
	for key: String in harvest_node_info.keys():
		if key in inventory.keys():
			var gatherable = min(harvest_node_info[key].value, INVENTORY_CAP) #
			inventory[key] += gatherable # add item to worker inventory
			harvest_node_info[key].value -= gatherable
